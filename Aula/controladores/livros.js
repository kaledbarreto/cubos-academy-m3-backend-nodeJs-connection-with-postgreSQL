...
const cadastrarLivro = async (req, res) => {
  ...
  try {
    ...
    if (livroCadastrado.rowCount === 0) {
      return res.status(400).json('Não foi possível cadatrar o livro');
    }
...
const atualizarLivro = async (req, res) => {
  ...
  try {
    ...
    if (livroAtualizado.rowCount === 0) {
      return res.status(400).json('Não foi possivel atualizar o livro');
    }
...
const excluirLivro = async (req, res) => {
  ...
  try {
    ...
    if (livroExcluido.rowCount === 0) {
      return res.status(400).json('Não foi possivel excluir o livro');
    }
