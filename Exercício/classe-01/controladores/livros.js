...
const excluirLivro = async (req, res) => {
    ...
    try {
        ...
        JOIN emprestimos ON livros.id = emprestimos.usuario_id
        WHERE livros.id = $1`;
        const livroDelete = await conexao.query(query, [id]);

        if (livroDelete.rowCount === 0) {
            await conexao.query('DELETE FROM livros WHERE id = $1', [id]);

            return res.status(200).json('O livro foi excluido com sucesso.');
        }

        return res.status(400).json('Não foi possível excluir o livro pois ele tem um empréstimo atrelado.')
    } catch (error) {
        return res.status(400).json(error.message);
    }
}
